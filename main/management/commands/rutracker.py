...
class Command(BaseCommand):
    help = "Поиск на RuTracker"

    def add_arguments(self, parser):
        parser.add_argument(
            "query", type=str, help="Поисковый запрос для RuTracker"
        )

    def handle(self, *args, **kwargs):
        query = kwargs["query"]
        self.stdout.write(f"Выполняется поиск на RuTracker для запроса: {query}")

        try:
            engine = RuTracker()
            engine.search(query)
            self.stdout.write("Поиск завершен успешно.")
        except Exception as e:
            self.stderr.write(f"Ошибка при выполнении поиска: {e}")
